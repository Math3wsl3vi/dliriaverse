...
const HomePage = () => {
  return (
    <div className='mt-5 p-3 grid grid-cols-1 md:grid-cols-3 gap-5 '>
      {uploadData.map((item)=>(
        <div key={item.id} className='mb-3 hover:shadow-sm hover:rounded-md pb-4 cursor-pointer' >
          <h1 className='font-poppins pl-2'>{item.name}</h1>
          <p className='font-poppins pl-2'>{item.county}</p>
          <div className='flex items-center justify-center'>
          <Image src={item.imageUrl} alt={item.name} width={300} height={300} className='w-[95%] md:w-1/3 rounded-md'/>
          </div>
          <div className='flex flex-row justify-between items-center mt-2'>
            <div className='flex gap-3'>
              <div className='pl-2 grid grid-cols-2 items-center gap-1'>
            ...
            <div className='pr-2 grid grid-cols-2 items-center gap-1'>
