...
const PostsPart = () => {
  ...
  const handleDeletePost = async () => {
    if (postToDelete) {
      try {
        const postDocRef = doc(firestore, "posts", postToDelete);
        ...
        {posts.map((item) => (
          ...
          >
            <Image
              ...
              width={300}
              height={200}
              className="w-full h-[400px] rounded-md object-cover md:w-[500px] md:h-[300px]"
            />
                ...
                <Dialog.Root open={postToDelete === item.id} onOpenChange={(open) => open ? setPostToDelete(item.id) : setPostToDelete(null)}>
                  ...
                  <Dialog.Portal>
                    ...
                    <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg w-96">
                      <Dialog.Title className="font-semibold text-lg font-poppins">Confirm Deletion</Dialog.Title>
                      <Dialog.Description className="mt-2 text-sm font-poppins">
                        Are you sure you want to delete this post?
                      </Dialog.Description>
                      <div className="mt-4 flex justify-between">
                        <Dialog.Close asChild>
                          <button className="text-gray-500 font-poppins">Cancel</button>
                        </Dialog.Close>
                        <button
                          onClick={handleDeletePost}
                          className="text-red-500 font-semibold font-poppins"
                        >
                          Confirm
                        </button>
                      </div>
